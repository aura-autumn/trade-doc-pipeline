...
**Nova Platform · GoComet · Full-Stack AI Engineer DAW (Part 1)**

> Execution-oriented. An engineer should be able to read this and start building. The POC in this repo already implements Sections 4–7.
...
So a crash after any node leaves a re-readable, consistent record — you can resume from persisted rows rather than re-running upstream work. LangGraph's checkpointer (`thread_id = shipment_id`) additionally gives in-process resume. **Honest limitation:** the checkpointer is currently `MemorySaver` (in-memory), so cross-*process* resume relies on the SQLite writes, not the checkpointer. Upgrade path: swap in the SQLite/Postgres checkpointer for durable graph-level recovery (see §8).
...
- **Avoid it:** the **validation rules** and the **routing decision** are plain Python — no LLM, no function-calling. Anything that determines whether a document is *correct* or *approved* must be deterministic and testable, never model-generated.
...
I'd pick these over, say, a prettier UI or more document types because **trust and the real trigger** are what determine whether the "boring 80%" actually gets automated — everything else is polish on top of an unproven core.
