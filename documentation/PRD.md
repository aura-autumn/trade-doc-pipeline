...
- **Extractor (vision fallback):** we use tesseract for images locally. It is fast and small and does the job well. for fallback, a **vision-capable** model Gemini 2.0 Flash (multimodal) / GPT-4o / LLaVA (local) invoked when the document is a scan/image or the text layer is empty.
